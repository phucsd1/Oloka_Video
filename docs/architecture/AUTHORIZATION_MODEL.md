...
| AUTH-042 | Admin: owner-facing Project/Composition/Job/SSE    | Deny; use redacted `/api/v1/admin/jobs` list/detail only for Job diagnostics        |
| AUTH-043 | Admin: disable/reject/demote an active admin       | Deny when the transition would leave no active admin                                |
| AUTH-044 | System: first-admin bootstrap                      | Allow only verified Google identity matching configured email while no admin exists |
...
| GenerationJob/RenderJob     | Deny                                      | Own jobs/status/history/SSE only    | Redacted admin list/detail/command; no owner SSE                                 | Matching lease only                          |
...
Authorization tests must prove cross-user denial for Project, Asset,
CompositionVersion, parent/child Job, and RenderOutput; admin denial on
owner-facing Project/Composition/Job-history/SSE routes; redacted admin Job
list/detail; pending/disabled/rejected denial with distinct account codes;
last-active-admin protection; verified first-admin bootstrap; invalid or expired
lease denial; credential redaction; traversal rejection before storage access;
and purge unreachability from ordinary HTTP handlers. Acceptance criteria are
indexed in `../product/ACCEPTANCE_CRITERIA.md`.
