...
Actors: `visitor`, `pending_user`, `active_member`, `disabled_user`, `admin`, `system_worker`. A `rejected` user has the same product-route denial as a disabled user and may only access the minimum account/logout surface.
...
| AUTH-013 | Active member: reference Asset in Composition       | Allow only when Asset is `ready` and belongs to the same Project                   |
...
| AUTH-019 | Active member: read own GenerationJob/status        | Allow through Project/requester lineage                                            |
| AUTH-020 | Active member: another user's GenerationJob         | Deny, including diagnostics and events                                             |
...
| AUTH-024 | Active member: playback/download own RenderOutput   | Allow only when `verified` and technical QA passed                                 |
| AUTH-025 | Active member: another user's RenderOutput          | Deny before signing or revealing object location                                   |
| AUTH-026 | Disabled/rejected user: authenticated product route | Deny immediately even when session is not expired                                  |
...
| RenderOutput                | Deny                                      | Own verified output only            | No default playback                                                              | Verify/purge under job capability            |
...
Authorization tests must prove cross-user denial for Project, Asset, CompositionVersion, Job, and RenderOutput; pending/disabled denial; invalid lease denial; credential redaction; traversal rejection before storage access; and purge unreachability from ordinary HTTP handlers. Acceptance criteria are indexed in `../product/ACCEPTANCE_CRITERIA.md`.
